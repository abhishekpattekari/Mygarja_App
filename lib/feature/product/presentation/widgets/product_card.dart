  ...
  Widget build(BuildContext context) {
    return Consumer<ProductController>(
      builder: (context, productController, child) {
        // Find the product ID by matching the properties
        int? productId;
        try {
          final product = productController.products.firstWhere(
            (p) => p.name == title && p.price == price && p.imageUrl == image_url && p.category == category
          );
          productId = product.id;
        } catch (e) {
          // If product not found, we'll pass the data directly
          productId = null;
        }
        ...
        return InkWell(
          onTap: () {
            if (productId != null) {
              ...
              // Fallback to passing data directly
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => ProductDetailScreen(
